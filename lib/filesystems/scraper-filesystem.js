  ...
  async list(options) {
    ...
    try {
      ...
      async function scanner(path) {
        ...
        let content = await that.getHttpPage(path);
  ...
  */
  ...
  async getHttpFile(path) {
    ...
    try {
      ...
      if (this.options.saveFiles) {
        await download.saveAs(this.options.savePath + filename);
      }
