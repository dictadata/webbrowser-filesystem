  ...
  async createReadStream(options) {
    ...
    try {
      ...
      if (ext === '.json' || ext === '.xml')
        ...
        rs = await this.getHttpContent(options);
      else
        // browser download
  ...
  */
  ...
  async download(options) {
    ...
    try {
      ...
      let ext = path.extname(options.href);
      if (ext === '.json' || ext === '.xml')
        // browser displays file content
        result = await this.getHttpContent(options);
      else
        // browser download
  ...
  async getHttpContent(options) {
    let rs = null;

    await this.page.click('a[href="' + encodeURI(options.href) + '"]', { button: "left" });
    ...
    if (options.saveFiles) {
      fs.writeFileSync(options.saveFolder + options.name, content);
      rs = true;
    }
    else {
      var rs = new Readable();
  ...
  async getHttpFile(options) {
    ...
    try {
      const [download] = await Promise.all([
        ...
        this.page.click('a[href="' + encodeURI(options.href) + '"]', { button: "left" })
      ]);
