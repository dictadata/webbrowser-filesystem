  ...
  */
  ...
  async _getHttpContent(options) {
    ...
    if (options._saveFiles) {
      try {
        if (options.useRPath)
          fs.mkdirSync(options.downloadFolder + path.dirname(options.rpath));
      }
      catch (error) {
        if (error.code !== 'EEXIST')
          logger.error(error.messsage);
      }
      let filename = options.downloadFolder + (options.useRPath ? options.rpath : options.name);
  ...
  async _getHttpFile(options) {
    ...
    try {
      ...
      if (options._saveFiles) {
        let filename = options.downloadFolder + (options.useRPath ? options.rpath : name);
