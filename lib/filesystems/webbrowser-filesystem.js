...
module.exports = exports = class WebBrowserFileSystem extends FileSystem {
  ...
  */
  ...
  async download(options) {
    logger.debug("WebBrowserFileSystem download");
    options = Object.assign({}, this.options, options);
  ...
  async upload(options) {
    logger.debug("WebBrowserFileSystem upload")
    throw new StorageError({ statusCode: 501 }, "WebBrowserFileSystem.upload method not implemented");
  ...
  async _getHttpContent(options) {
    ...
    if (options._saveFiles) {
      try {
        if (options.useRPath)
          fs.mkdirSync(options.downloads + path.dirname(options.rpath));
      }
      catch (error) {
        if (error.code !== 'EEXIST')
          logger.error(error.messsage);
      }
      let filename = options.downloads + (options.useRPath ? options.rpath : options.name);
  ...
  async _getHttpFile(options) {
    ...
    try {
      ...
      if (options._saveFiles) {
        let filename = options.downloads + (options.useRPath ? options.rpath : name);
